...
import { createMutasiSiswa, deleteMutasiSiswa, getAllMutasiSiswa, updateMutasiSiswa } from "@/lib/model/mutasiSiswaModel"
import { logRiwayat } from "@/lib/model/riwayatModel"
import {   createMultiSiswa, createSingleSiswa, deleteSingleSiswaByNis,  naikkanKelasSiswa, naikkanKelasSiswa_selected, updateSiswaByNIS } from "@/lib/model/siswaModel"
...
export default function DataMutasiSiswaPage() {
    ...
    const getData = async () => {
        ...
        const response = await getAllMutasiSiswa()
        if(response.success) {
    ...
    const submitFormTambah = async (e, modal) => {
        ...
        Swal.fire({
            ...
            didOpen: async () => {
                const response = await createMutasiSiswa(payload)
                ...
                if(response) {
                    ...
                    Swal.fire({
                        title: 'Sukses',
                        icon: 'success',
                        text: 'Berhasil menambahkan data mutasi siswa',
                        timer: 3000,
                        timerProgressBar: true
                    })
                }else{
    ...
    const submitDeleteData = async (nis) => {
        Swal.fire({
            ...
            didOpen: async () => {
                ...
                if(nis) {
                    response = await deleteMutasiSiswa([nis])
                }else{
                    response = await deleteMutasiSiswa(selectedData)
                }
                ...
                if(response.success) {
                    ...
                    Swal.fire({
                        title: 'Sukses',
                        icon: 'success',
                        text: 'Berhasil menghapus data mutasi siswa'
                    })
                }else{
                    Swal.fire({
                        title: 'Gagal',
    ...
    const submitEditData = (e, modal, nis) => {
        ...
        Swal.fire({
            ...
            allowEscapeKey: false,
            didOpen: async () => {
                const response = await updateMutasiSiswa([nis], payload)
                ...
                if(response.success) {
                    ...
                    Swal.fire({
                        title: 'Sukses',
                        text: 'Berhasil mengubah data mutasi siswa tersebut',
                        icon: 'success'
                    })
                }else{
                    ...
                    Swal.fire({
                        ...
                        didOpen: async () => {
                            await logRiwayat({
                                aksi: 'Export',
                                kategori: 'Data Mutasi Siswa',
                                keterangan: `Mengexport 1 data sebagai PDF`,
                    ...
                    Swal.fire({
                        ...
                        didOpen: async () => {
                            await logRiwayat({
                                aksi: 'Export',
                                kategori: 'Data Mutasi Siswa',
                                keterangan: `Mengexport ${dataSiswa.length} data sebagai PDF`,
    ...
    const submitAktifkanSiswa = (nis, modal) => {
        ...
        Swal.fire({
            ...
            didOpen: async () => {
                ...
                if(nis) {
                    const {tahun_keluar, tanggal_keluar, keterangan, ...updatedDataSiswa} = data.find(value => value['nis'] === nis)
                    dataSiswa = updatedDataSiswa
                    responseSiswa = await createSingleSiswa(dataSiswa)
                    responseMutasiSiswa = await deleteMutasiSiswa([dataSiswa['nis']])
                }else{
                    const updatedDataSiswa = data.filter(value => selectedData.includes(value['nis'])).map(value => {
                        const {tahun_keluar, tanggal_keluar, keterangan, ...obj} = value
                        return obj
                    })
                    dataSiswa = updatedDataSiswa
                    responseSiswa = await createMultiSiswa(dataSiswa)
                    responseMutasiSiswa = await deleteMutasiSiswa(dataSiswa.map(value => value['nis']))
                }
                ...
                if(responseMutasiSiswa.success && responseSiswa.success) {
                    await logRiwayat({
                        aksi: 'Mutasi',
                        kategori: 'Data Mutasi Siswa',
                        keterangan: `Mengaktifkan ${nis ? '1' : dataSiswa.length} data ke dalam Data Siswa`,
                        records: JSON.stringify(nis ? { nis } : { nis: dataSiswa.map(value => value['nis'])})
                    })
                    setSelectAll(false)
                    setSelectedData([])
                    await getData()
                    Swal.fire({
                        title: 'Sukses',
                        text: 'Data mutasi siswa tersebut berhasil di aktifkan kembali!',
                        icon: 'success'
                    })
                }else{
                    Swal.fire({
                            ...
                            <div className="modal-box bg-white dark:bg-zinc-900 rounded md:max-w-[900px] border dark:border-zinc-800">
                                ...
                                <form onSubmit={e => submitFormTambah(e, 'tambah_siswa')} className="space-y-5 md:space-y-3">
                                    ...
                                    <div className="flex flex-col md:flex-row md:items-center gap-2">
                                        ...
                                        <div className="w-full md:w-2/3">
                                            <input required type="text" className="px-3 py-2 rounded-md w-full bg-transparent border dark:border-zinc-800" placeholder="Keterangan Keluar" />
                                        </div>
                                    </div>
                        ...
                        {filteredData.slice(pagination === 1 ? totalList - totalList : (totalList * pagination) - totalList, totalList * pagination).map((value, index) => (
                            <div key={`${value['nis']}`} className="grid grid-cols-12 p-3 rounded-lg hover:bg-zinc-50 dark:hover:bg-zinc-800 ease-out duration-300 text-xs">
                                ...
                                <div className="col-span-5 md:col-span-2 flex items-center justify-center md:gap-3 gap-1">
                                    <button type="button" onClick={() => document.getElementById(`info_mutasi_${value['nis']}`).showModal()} className="w-6 h-6 rounded border dark:border-zinc-700 bg-zinc-100 dark:bg-zinc-800 flex items-center justify-center hover:border-blue-500 dark:hover:border-blue-500/50 hover:bg-blue-100 dark:hover:bg-blue-500/10 hover:text-blue-600 dark:hover:text-blue-500 ease-out duration-200">
                                        <FontAwesomeIcon icon={faFile} className="w-3 h-3 text-inherit" />
                                    </button>
                                    <dialog id={`info_mutasi_${value['nis']}`} className="modal bg-gradient-to-t dark:from-zinc-950 from-zinc-50">
                                        <div className="modal-box bg-white dark:bg-zinc-900 rounded  border dark:border-zinc-800 max-w-[800px]">
                                            <form method="dialog">
                                                <button className="btn btn-sm btn-circle btn-ghost absolute right-2 top-2">✕</button>
                                            </form>
                                            <h3 className="font-bold text-lg">Informasi Mutasi Siswa</h3>
                                            <hr className="my-2 opacity-0" />
                                            <div className="flex flex-col md:flex-row md:items-center gap-3">
                                                
                                                <button type="button" onClick={() => handlePrintData(`info_mutasi_${value['nis']}`, value['nis'])} className="w-full md:w-fit px-3 py-2 border rounded-md dark:border-zinc-700 hover:bg-zinc-100 dark:hover:bg-zinc-800 flex items-center justify-center gap-3">
                                                    <FontAwesomeIcon icon={faPrint} className="w-3 h-3 text-inherit opacity-60" />
                                                    Print
                                                </button>
                                                <button type="button" onClick={() => submitAktifkanSiswa(value['nis'], `info_mutasi_${value['nis']}`)} className="w-full md:w-fit px-3 py-2 border rounded-md dark:border-zinc-700 hover:bg-zinc-100 dark:hover:bg-zinc-800 flex items-center justify-center gap-3">
                                                    <FontAwesomeIcon icon={faPowerOff} className="w-3 h-3 text-green-500 opacity-60" />
                                                    Aktifkan 
                                                </button>
                                            </div>
                                            <hr className="my-2 opacity-0" />
                                    ...
                                    <button type="button" onClick={() => document.getElementById(`edit_mutasi_${value['nis']}`).showModal()} className="w-6 h-6 rounded border dark:border-zinc-700 bg-zinc-100 dark:bg-zinc-800 flex items-center justify-center hover:border-amber-500 dark:hover:border-amber-500/50 hover:bg-amber-100 dark:hover:bg-amber-500/10 hover:text-amber-600 dark:hover:text-amber-500 ease-out duration-200">
                                        <FontAwesomeIcon icon={faEdit} className="w-3 h-3 text-inherit" />
                                    </button>
                                    <dialog id={`edit_mutasi_${value['nis']}`} className="modal bg-gradient-to-t dark:from-zinc-950 from-zinc-50">
                                        <div className="modal-box bg-white dark:bg-zinc-900 rounded  border dark:border-zinc-800 max-w-[800px]">
                                            <form method="dialog">
                                                <button className="btn btn-sm btn-circle btn-ghost absolute right-2 top-2">✕</button>
                                            </form>
                                            ...
                                            <hr className="my-2 opacity-0" />
                                            <form onSubmit={(e) => submitEditData(e, `edit_mutasi_${value['nis']}`, value['nis'])} className="space-y-6 md:space-y-3">
                                                <div className="flex flex-col md:flex-row md:items-center gap-2">
                                                    <p className="opacity-60 w-full md:w-1/3">
                                                        Nama Siswa
                                                    </p>
                                                ...
                                                <div className="flex flex-col md:flex-row md:items-center gap-2">
                                                    ...
                                                    <div className="w-full md:w-2/3">
                                                        <input required defaultValue={value['keterangan']} type="text" className="px-3 py-2 rounded-md w-full bg-transparent border dark:border-zinc-800" placeholder="Keterangan Keluar" />
                                                    </div>
                                                </div>
                                                <div className="flex md:justify-end">
                                                    <button type="submit" className="w-full md:w-fit px-3 py-2 rounded-md flex items-center justify-center gap-3 bg-green-500 hover:bg-green-400 focus:bg-green-600 text-white">
