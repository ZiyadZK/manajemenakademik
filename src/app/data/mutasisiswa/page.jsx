    ...
    return (
        <MainLayoutPage>
            ...
            <div className="flex items-center md:gap-5 w-full justify-center md:justify-start gap-2">
                <a href="/data/mutasisiswa/new" className={`${rale.className} rounded-full px-4 py-2 bg-zinc-100 text-zinc-700 font-medium hover:bg-zinc-200 md:text-xl flex items-center justify-center gap-2 dark:bg-zinc-700/50 dark:hover:bg-zinc-700 dark:text-zinc-400 dark:hover:text-zinc-200`}>
                    <FontAwesomeIcon icon={faPlus} className="w-4 h-4 text-inherit" />
                    Tambah Data
                </a>
                <a href="/data/mutasisiswa/new/import" className={`${rale.className} rounded-full px-4 py-2 bg-zinc-100 text-zinc-700 font-medium hover:bg-zinc-200 md:text-xl flex items-center justify-center gap-2 dark:bg-zinc-700/50 dark:hover:bg-zinc-700 dark:text-zinc-400 dark:hover:text-zinc-200`}>
                    <FontAwesomeIcon icon={faDownload} className="w-4 h-4 text-inherit" />
                    Import Data
                </a>
            </div>
            <hr className="my-1 md:my-2 opacity-0" />
            <div className="p-5 rounded-2xl bg-zinc-50 md:bg-zinc-100 text-zinc-800 dark:bg-zinc-700/20 dark:text-zinc-200">
                ...
                <div className="flex md:flex-row flex-col gap-5">
                    <div className="w-full md:w-1/2 flex gap-2">
                        <select value={kelas} onChange={e => setKelas(e.target.value)} className="w-1/2 px-2 py-1 rounded-xl border bg-white text-xs md:text-sm cursor-pointer dark:bg-zinc-700/50 dark:border-zinc-800">
                            {listKelas.map((kelasItem, index) => (
                                <option key={index} value={kelasItem}>{kelasItem}</option>
                            ))}
                            <option value="">Semua Kelas</option>
                        </select>
                        <select value={rombel} onChange={e => setRombel(e.target.value)} className="w-1/2 px-2 py-1 rounded-xl border bg-white text-xs md:text-sm cursor-pointer dark:bg-zinc-700/50 dark:border-zinc-800">
                    ...
                    <div className="w-full md:w-1/2 flex gap-2">
                        <select value={noRombel} onChange={e => setNoRombel(e.target.value)} className="w-1/2 px-2 py-1 rounded-xl border bg-white text-xs md:text-sm cursor-pointer dark:bg-zinc-700/50 dark:border-zinc-800">
                            {listNoRombel.map((no_rombel, index) => (
                                <option key={index} value={no_rombel}>{no_rombel}</option>
                            ))}
                            <option value="">Semua No Rombel</option>
                        </select>
                        <select value={status} onChange={e => setStatus(e.target.value)} className="w-1/2 px-2 py-1 rounded-xl border bg-white text-xs md:text-sm cursor-pointer dark:bg-zinc-700/50 dark:border-zinc-800">
            ...
            <div className="flex items-center gap-5 w-full">
                <input type="text" onChange={e => setSearchValue(e.target.value)} className=" bg-zinc-100 flex-grow md:flex-grow-0 md:w-80 px-3 py-2 text-xs md:text-lg rounded-xl border bg-transparent dark:bg-zinc-700/50 dark:border-zinc-800 dark:text-zinc-200" placeholder="Cari data anda disini" />
                <select value={searchCriteria} onChange={e => setSearchCriteria(e.target.value)}  className=" px-3 py-2 rounded-xl border text-xs md:text-lg bg-white  cursor-pointer dark:bg-zinc-700/50 dark:border-zinc-800 dark:text-zinc-200">
            ...
            <div className="grid grid-cols-12 w-full  bg-blue-500 dark:bg-blue-500/70 *:px-2 *:py-3 text-white  text-sm shadow-xl">
                ...
                <div className="relative w-full h-fit max-h-[300px] overflow-auto">
                    <div className="divide-y dark:divide-zinc-700">
                        {filteredSiswaList.slice(pagination === 1 ? totalList - totalList : (totalList * pagination) - totalList, totalList * pagination).map((siswa) => (
                            <div key={siswa.nis} className="grid grid-cols-12 w-full  hover:bg-zinc-100 *:px-2 *:py-3 text-zinc-800 font-medium text-xs divide-x dark:divide-zinc-700 dark:text-zinc-200 dark:hover:bg-zinc-700/20">
                                <div className="flex items-center gap-3 col-span-8 md:col-span-4 place-items-center">
                                    ...
                                    <button type="button" title="Aktifkan Siswa" onClick={() => document.getElementById(`${siswa.nama_siswa} - mutasi ${siswa.nis}`).showModal()} className="swap hover:swap-active focus:swap-active">
                                        <div className={`md:px-2 md:py-1 rounded-full swap-on bg-green-50 text-green-500 text-xs flex justify-center items-center dark:bg-green-500/10`}>
                                        ...
                                        <div className={`md:px-2 md:py-1 rounded-full swap-off bg-red-50 text-red-500 text-xs flex justify-center items-center dark:bg-red-500/10`}>
                                    ...
                                    <dialog id={`${siswa.nama_siswa} - mutasi ${siswa.nis}`} className="modal">
                                        <div className="modal-box dark:bg-zinc-800">
                                            ...
                                            <div>
                                                <h3 className="font-bold md:text-lg">Aktifkan <span className="bg-zinc-100 dark:bg-zinc-200/10 rounded-full font-normal tracking-tighter px-2 py-1">{siswa.nama_siswa}</span> </h3>
                                                <p className="py-4 md:text-sm">Anda akan mengaktifkan kembali siswa ini, dan data siswa ini akan dimasukkan ke Data Siswa yang <span className="text-green-700 dark:text-green-500">Aktif</span></p>
            ...
            <div className="w-full flex md:items-center md:justify-between px-2 py-1 flex-col md:flex-row border-y border-zinc-300 dark:border-zinc-700">
                <div className="flex-grow flex justify-between items-center">
                    <div className="flex items-center gap-2">
                        ...
                        <button type="button" onClick={() => deleteSelectedSiswa()} className={`w-7 h-7 ${selectedSiswa && selectedSiswa.length > 0 ? 'flex' : 'hidden'} items-center justify-center rounded-lg bg-zinc-100 hover:bg-zinc-200 text-zinc-500 focus:bg-red-200 focus:text-red-700 dark:bg-zinc-200/10 dark:hover:bg-zinc-200/20 dark:hover:text-zinc-200`}>
                            <FontAwesomeIcon icon={faTrash} className="w-3 h-3 text-inherit" />
                        </button>
                        <button type="button" onClick={() => setShowSelected(state => !state)} className={`w-7 h-7 flex items-center justify-center rounded-lg   ${showSelected ? 'bg-blue-200 text-blue-700 hover:bg-blue-300 dark:bg-blue-500/10 dark:hover:bg-blue-300/20' : 'text-zinc-500 bg-zinc-100 hover:bg-zinc-200 dark:bg-zinc-200/10 dark:hover:bg-zinc-200/20 dark:hover:text-zinc-200'} group transition-all duration-300`}>
                            <FontAwesomeIcon icon={faEye} className="w-3 h-3 text-inherit group-hover:scale-125 transition-all duration-300" />
                        </button>
                        <button type="button" onClick={() => setSelectedSiswa([])} className={`w-7 h-7 ${selectedSiswa && selectedSiswa.length > 0 ? 'flex' : 'hidden'} items-center justify-center rounded-lg  group transition-all duration-300 bg-zinc-100 hover:bg-zinc-200 dark:bg-zinc-200/10 dark:hover:bg-zinc-200/20 dark:hover:text-zinc-200 dark:text-zinc-500 `}>
                                    ...
                                    <div className="">
                                        ...
                                        <div className="p-3 rounded-lg border w-full flex flex-wrap gap-1 dark:border-zinc-700">
                                            {exportExcel.kolomDataArr.map((kolomData, index) => (
                                                <div key={`${index} - ${index}`} className="p-2 rounded bg-zinc-100 text-xs flex items-center justify-center gap-2 font-medium dark:bg-zinc-700/50 dark:text-zinc-200">
                                                    {kolomData['keyName']}
                ...
                <div className="w-full md:w-fit flex items-center justify-center divide-x mt-2 md:mt-0">
                    <p className={`${mont.className} px-2 text-xs dark:text-zinc-400`}>
                    ...
                    <div className={`${mont.className} px-2 text-xs`}>
                        <select  value={totalList} onChange={e => handleTotalList(e.target.value)} className="cursor-pointer px-2 py-1 hover:bg-zinc-100 rounded bg-transparent dark:hover:bg-zinc-700/50 dark:text-zinc-200">
            ...
            <div className="flex items-center gap-5">
                <div className="flex items-center gap-3">
                    <p className="text-xs opacity-70 dark:text-orange-500">
                        Socket Server:
                    </p>
                    {statusSocket === '' && (
            ...
            <div className="md:p-5 mb-10 rounded-xl md:border border-zinc-400 flex flex-col md:flex-row gap-5 transition-all duration-300 dark:border-zinc-700">
                ...
                <form onSubmit={submitUpdateBersama} className="w-full md:w-1/2">
                    ...
                    <div className={`${selectedSiswa && selectedSiswa.length > 0 ? 'flex' : 'hidden'} items-center gap-3`}>
                        ...
                        <button type="button" onClick={() => setSelectedSiswa([])}  className="px-3 py-2 rounded-full bg-zinc-100 text-zinc-700 font-medium md:hidden flex items-center justify-center gap-3 text-sm hover:bg-zinc-200 hover:text-zinc-800 dark:bg-zinc-700/50 dark:text-zinc-400 dark:hover:bg-zinc-700 dark:hover:text-zinc-200">
