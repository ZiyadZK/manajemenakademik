...
const formatDataPribadi = ['kelas', 'rombel', 'no_rombel', 'nama_siswa', 'nis', 'nisn', 'nik', 'no_kk', 'tempat_lahir', 'tanggal_lahir', 'jenis_kelamin', 'agama', 'status_dalam_keluarga', 'anak_ke', 'alamat', 'no_hp_siswa', 'asal_sekolah', 'kategori', 'tahun_masuk', 'tahun_keluar', 'tanggal_keluar', 'keterangan']
...
const formatDataSiswa = ['kelas', 'rombel', 'no_rombel', 'nama_siswa', 'nis', 'nisn', 'nik', 'no_kk', 'tempat_lahir', 'tanggal_lahir', 'jenis_kelamin', 'agama', 'status_dalam_keluarga', 'anak_ke', 'alamat', 'no_hp_siswa', 'asal_sekolah', 'kategori', 'tahun_masuk', 'tahun_keluar', 'tanggal_keluar', 'keterangan', 'nama_ayah', 'nama_ibu', 'telp_ortu', 'pekerjaan_ayah', 'pekerjaan_ibu']
const formatInformasiFile = {status: '', ekstensi: '', size: '', jumlahData: ''}
const mySwal = withReactContent(Swal)

export default function DataMutasiSiswaNewImportPage() {
    ...
    const downloadFormatExample = async type => {
        if(type === 'xlsx') {
            return await exportToXLSX([], 'Format Data Mutasi Siswa', {
        ...
        if(type === 'csv') {
            return await exportToCSV([], 'Format Data Mutasi Siswa', {
                header: formatDataSiswa,
        ...
        mySwal.fire({
            ...
            if(result.isConfirmed) {
                mySwal.fire({
                    ...
                    timer: 15000,
                    didOpen: async () => {
                        const response = await model_createAlumni(data);
                        if(response.success) {
                            mySwal.fire({
                                icon: 'success',
                                title: 'Berhasil memproses data!',
                                text: 'Berhasil mengupload data import ke data mutasisiswa!',
                                timer: 2000,
                                showConfirmButton: false
    ...
    const handleSearchFilter = (value) => {
        ...
        const updatedData = data.filter((siswa, index, array) => 
            ...
            String(siswa['tahun_keluar']).toLowerCase().includes(value.toLowerCase()) ||
            ...
            <div className="relative w-full h-fit max-h-[300px] divide-y overflow-auto">
                {filteredData.slice(pagination === 1 ? totalList - totalList : (totalList * pagination) - totalList, totalList * pagination).map(siswa => (
                    <div key={siswa.nis} className="grid grid-cols-12 w-full  hover:bg-zinc-100 *:px-2 *:py-3 text-zinc-800 font-medium text-xs divide-x">
                        ...
                        <div className="hidden md:flex items-center col-span-2 gap-3">
                            {siswa.tahun_masuk} - {siswa.tahun_keluar}
                        </div>
