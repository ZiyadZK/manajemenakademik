...
import { dateToIso } from "@/lib/dateConvertes"
import { createSinglePegawai, getSinglePegawai, updateSinglePegawai } from "@/lib/model/pegawaiModel"
import { getDataSertifikat } from "@/lib/model/sertifikatModel"
import { faAngleLeft, faArrowLeft, faDownload, faEdit, faPlus, faSave, faUserPlus, faXmark } from "@fortawesome/free-solid-svg-icons"
...
export default function DataPegawaiUpdatePage({params}) {
    ...
    const submitFormPegawai = async (e) => {
        ...
        mySwal.fire({
            ...
            icon: 'question',
            text: 'Anda akan mengubah data pegawai',
            ...
            if(result.isConfirmed) {
                Swal.fire({
                    ...
                    didOpen: async () => {
                        const response = await updateSinglePegawai(formPegawai)
                        if(response.success) {
                            Swal.fire({
                                title: 'Sukses',
                            ...
                            Swal.fire({
                                title: 'Error',
                                text: 'Gagal mengubah data pegawai tersebut!',
                                icon: 'error'
                            })
                        }
                    }
    ...
    const addSertifikat = (jenis_sertifikat) => {
        ...
        setDataSertifikat(state => ([...state, newForm]))
    }

    const updateSertifikat = async (sertifikat_id, payload) => {
        // Find the index of the certificate in the array
        const sertifikatIndex = dataSertifikat.findIndex(sertifikat => sertifikat.sertifikat_id === sertifikat_id);
        console.log(sertifikatIndex)
        ...
        if (sertifikatIndex !== -1) {
            ...
            const updatedSertifikat = {
                ...dataSertifikat[sertifikatIndex],
                ...payload
            };
            console.log(updatedSertifikat)
    
            // Create a new array with the updated certificate
            const updatedSertifikatArray = [...dataSertifikat];
            updatedSertifikatArray[sertifikatIndex] = updatedSertifikat;
            console.log(updatedSertifikatArray)
    
            // Update the state with the new array
            setDataSertifikat(updatedSertifikatArray)
        }
    };

    const deleteSertifikat = (index) => {
        console.log(index)
        const updatedDataSertifikat = [
            ...dataSertifikat.slice(0, index),
            ...dataSertifikat.slice(index + 1)
        ]

        // Set the state with the updated formPegawai object
        setDataSertifikat(updatedDataSertifikat)
    }
                            ...
                            <div className="w-full md:w-1/2 md:pr-5">
                                ...
                                <div className="space-y-3 mt-2">
                                    ...
                                    <div className="flex md:flex-row flex-col gap-1 md:gap-0 md:items-center">
                                        ...
                                        <div className="w-full md:w-3/5 space-y-2 md:space-y-1">
                                            <select value={formPegawai.jabatan} onChange={e => setFormPegawai(state => ({...state, jabatan: e.target.value}))} className={`${mont.className} border px-2 py-1 rounded w-full text-sm outline-none cursor-pointer bg-white hover:outline-zinc-200 focus:outline-zinc-400`}>
                                    ...
                                    <div className="flex md:flex-row flex-col gap-1 md:gap-0 md:items-center">
                                        ...
                                        <div className="w-full md:w-3/5">
                                            <input value={formPegawai.nik} onChange={e => setFormPegawai(state => ({...state, nik: e.target.value}))} type="text" className={" bg-white hover:outline-zinc-200 focus:outline-zinc-400 border px-2 py-1 rounded w-full text-sm outline-none " + mont.className} placeholder="NIP" />
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex md:flex-row flex-col gap-1 md:gap-0 md:items-center">
                                        ...
                                        <div className="w-full md:w-3/5">
                                            <input value={formPegawai.nip} onChange={e => setFormPegawai(state => ({...state, nip: e.target.value}))} type="text" className={" bg-white hover:outline-zinc-200 focus:outline-zinc-400 border px-2 py-1 rounded w-full text-sm outline-none " + mont.className} placeholder="NIP" />
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex md:flex-row flex-col gap-1 md:gap-0 md:items-center">
                                        ...
                                        <div className="w-full md:w-3/5">
                                            <input value={formPegawai.nuptk} type="text" onChange={e => setFormPegawai(state => ({...state, nuptk: e.target.value}))} className={" bg-white hover:outline-zinc-200 focus:outline-zinc-400 border px-2 py-1 rounded w-full text-sm outline-none " + mont.className} placeholder="NUPTK" />
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex md:flex-row flex-col gap-1 md:gap-0 md:items-center">
                                        ...
                                        <div className="flex items-end flex-col w-full md:w-3/5 gap-1">
                                            <select value={formPegawai.status_kepegawaian} onChange={e => setFormPegawai(state => ({...state, status_kepegawaian: e.target.value}))} className={`bg-white hover:outline-zinc-200 focus:outline-zinc-400 ${mont.className} border px-2 py-1 rounded w-full text-sm outline-none cursor-pointer`}>
                                    ...
                                    <div className="flex md:flex-row flex-col gap-1 md:gap-0 md:items-center">
                                        ...
                                        <input value={dateToIso(formPegawai.tgl_lahir)} onChange={e => setFormPegawai(state => ({...state, tgl_lahir: e.target.value}))} type="date" className={`${mont.className} bg-white hover:outline-zinc-200 focus:outline-zinc-400 border px-2 py-1 rounded w-full text-sm outline-none cursor-pointer md:w-3/5`} />
                                    </div>

                                    <div className="flex md:flex-row flex-col gap-1 md:gap-0 md:items-center">
                                        <p className="text-xs md:text-md font-medium text-zinc-400 w-full md:w-2/5">Tempat Lahir</p>
                                        <div className="w-full md:w-3/5">
                                            <input type="text" value={formPegawai.tmpt_lahir} onChange={e => setFormPegawai(state => ({...state, tmpt_lahir: e.target.value}))} className={"bg-white hover:outline-zinc-200 focus:outline-zinc-400 border px-2 py-1 rounded w-full text-sm outline-none " + mont.className} placeholder="Tempat Lahir" />
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-2 mt-2">
                                    ...
                                    <div className="flex md:flex-row flex-col gap-1 md:gap-0 md:items-center">
                                        ...
                                        <select value={formPegawai.pendidikan_terakhir} onChange={e => setFormPegawai(state => ({...state, pendidikan_terakhir: e.target.value}))} className={`${mont.className} bg-white hover:outline-zinc-200 focus:outline-zinc-400 border px-2 py-1 rounded w-full text-sm outline-none cursor-pointer md:w-3/5`}>
                                    ...
                                    <div className="space-y-1">
                                        ...
                                        <input type="text" value={formPegawai.sekolah_pendidikan} onChange={e => setFormPegawai(state => ({...state, sekolah_pendidikan: e.target.value}))} className={"bg-white hover:outline-zinc-200 focus:outline-zinc-400 border px-2 py-1 rounded w-full text-sm outline-none " + mont.className} placeholder="Isi Jika Ada" />
                                    </div>
                                    <div className="flex items-center gap-5">
                                        <h1 className="font-bold text-sm text-zinc-500">
                                            Sarjana
                                    ...
                                    <div className="space-y-1">
                                        ...
                                        <input type="text" value={formPegawai.sarjana_universitas} onChange={e => setFormPegawai(state => ({...state, sarjana_universitas: e.target.value}))} className={"bg-white hover:outline-zinc-200 focus:outline-zinc-400 border px-2 py-1 rounded w-full text-sm outline-none " + mont.className} placeholder="Isi Jika Ada" />
                                    </div>
                                    <div className="grid grid-cols-2 gap-5">
                                        <div className="space-y-1">
                                            <p className="text-xs font-bold">Fakultas</p>
                                            <input type="text" value={formPegawai.sarjana_fakultas} onChange={e => setFormPegawai(state => ({...state, sarjana_fakultas: e.target.value}))} className={"bg-white hover:outline-zinc-200 focus:outline-zinc-400 border px-2 py-1 rounded w-full text-sm outline-none " + mont.className} placeholder="Isi Jika Ada" />
                                        </div>
                                        <div className="space-y-1">
                                            <p className="text-xs font-bold">Program Studi</p>
                                            <input type="text" value={formPegawai.sarjana_prodi} onChange={e => setFormPegawai(state => ({...state, sarjana_prodi: e.target.value}))} className={"bg-white hover:outline-zinc-200 focus:outline-zinc-400 border px-2 py-1 rounded w-full text-sm outline-none " + mont.className} placeholder="Isi Jika Ada" />
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-5">
                                        <h1 className="font-bold text-sm text-zinc-500">
                                    ...
                                    <div className="space-y-1">
                                        <p className="text-xs font-bold">Nama Universitas</p>
                                        <input type="text" value={formPegawai.magister_universitas} onChange={e => setFormPegawai(state => ({...state, magister_universitas: e.target.value}))} className={"bg-white hover:outline-zinc-200 focus:outline-zinc-400 border px-2 py-1 rounded w-full text-sm outline-none " + mont.className} placeholder="Isi Jika Ada" />
                                    </div>
                                    <div className="grid grid-cols-2 gap-5">
                                        <div className="space-y-1">
                                            <p className="text-xs font-bold">Fakultas</p>
                                            <input type="text" value={formPegawai.magister_fakultas} onChange={e => setFormPegawai(state => ({...state, magister_fakultas: e.target.value}))} className={"bg-white hover:outline-zinc-200 focus:outline-zinc-400 border px-2 py-1 rounded w-full text-sm outline-none " + mont.className} placeholder="Isi Jika Ada" />
                                        </div>
                                        <div className="space-y-1">
                                            <p className="text-xs font-bold">Program Studi</p>
                                            <input type="text" value={formPegawai.magister_prodi} onChange={e => setFormPegawai(state => ({...state, magister_prodi: e.target.value}))} className={"bg-white hover:outline-zinc-200 focus:outline-zinc-400 border px-2 py-1 rounded w-full text-sm outline-none " + mont.className} placeholder="Isi Jika Ada" />
                                        </div>
                                    </div>
                                </div>
                                <hr className="my-3 opacity-0" />
                                        ...
                                        {dataSertifikat.map((sertifikat, index) => sertifikat.jenis_sertifikat === showCert && (
                                            <div key={`${index} - ${sertifikat.jenis_sertifikat}`} className="p-5 rounded-xl border space-y-2">
                                                ...
                                                <div className="flex flex-col md:flex-row md:items-center gap-1">
                                                    ...
                                                    <input type="text" required value={sertifikat.fileUrl} onChange={e => updateSertifikat(sertifikat.sertifikat_id, {fileUrl: e.target.value})} className={"bg-white px-2 py-1 hover:outline-zinc-200 focus:outline-zinc-400 border  rounded w-full text-sm outline-none " + mont.className} placeholder="Isi Jika Ada" />
                                                </div>
