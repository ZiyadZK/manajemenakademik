...
const formatDataIjazah = ['tgl_diambil', 'nama_lulusan', 'nisn', 'nama_pengambil', 'kelas', 'rombel', 'tahun_lulus', 'status']
const formatInformasiFile = {status: '', ekstensi: '', size: '', jumlahData: ''}
const mySwal = withReactContent(Swal)

export default function DataIjazahNewImportPage() {
    ...
    const submitFile = async e => {
        ...
        if(fileExtension === 'xlsx') {
            try {
                ...
                formatDataIjazah.filter(format => kolomInputDataArr.includes(format)).map(format => {
                    updatedInformasiKolom = {...updatedInformasiKolom, [format]: 'cocok'}
                })

                // Cek kolom yang tidak cocok
                formatDataIjazah.filter(format => kolomInputDataArr.includes(format) === false).map(format => {
    ...
    const downloadFormatExample = async type => {
        if(type === 'xlsx') {
            return await exportToXLSX([], 'Format Data Alumni', {
                header: formatDataIjazah,
                sheetName: 'Format XSLX'
            })
        }

        if(type === 'csv') {
            return await exportToCSV([], 'Format Data Alumni', {
                header: formatDataIjazah,
        ...
        mySwal.fire({
            ...
            if(result.isConfirmed) {
                mySwal.fire({
                    ...
                    timer: 15000,
                    didOpen: async () => {
                        const response = await createMultiIjazah(data);
                        if(response.success) {
                            mySwal.fire({
                                icon: 'success',
                                title: 'Berhasil memproses data!',
                                text: 'Berhasil mengupload data import ke data ijazah!',
                                timer: 2000,
                                showConfirmButton: false
                            }).then(() => router.push('/data/ijazah'));
                        }else{
                            mySwal.fire({
                                icon: 'error',
                                title: 'Gagal memproses data..',
                                text: 'Terdapat kendala disaat anda mengimport data ke data ijazah!',
                                timer: 2000,
                                allowOutsideClick: false
                            })
                        }
    ...
    const addSelectedSiswa = (nisn) => {
        if(selectedSiswa.includes(nisn)) {
            const updatedData = selectedSiswa.filter(siswaNis => siswaNis !== nisn);
            setSelectedSiswa(updatedData)
        }else{
            const updatedData = [...selectedSiswa, nisn]
            setSelectedSiswa(updatedData)
        }
    }

    const deleteSelectedSiswa = () => {
        const updatedData = data.filter(siswa => !selectedSiswa.includes(siswa.nisn));
        setSelectedSiswa([])
        setData(updatedData)
    ...
    const handleSearchFilter = (value) => {
        ...
        const updatedData = data.filter((siswa, index, array) => 
            siswa['nama_lulusan'].toLowerCase().includes(value.toLowerCase()) ||
            ...
            String(siswa['tahun_lulus']).toLowerCase().includes(value.toLowerCase()) ||
    ...
    const deleteSingleSiswa = (nisn) => {
        const updatedData = data.filter(siswa => siswa.nisn !== nisn)
        setData(updatedData)
    }
                    ...
                    <div key={siswa.nis} className="grid grid-cols-12 w-full  hover:bg-zinc-100 *:px-2 *:py-3 text-zinc-800 font-medium text-xs divide-x">
                        <div className="flex items-center gap-3 col-span-8 md:col-span-4 place-items-center">
                            <div className="flex-grow flex items-center gap-2">
                                <input type="checkbox" checked={selectedSiswa.includes(siswa.nisn)} onChange={() => addSelectedSiswa(siswa.nisn)} />
                                {siswa.nama_siswa}
                            </div>
                        ...
                        <div className="flex justify-center items-center  col-span-4 md:col-span-2 gap-1 md:gap-2">
                            <button type="button" onClick={() => document.getElementById(`informasi_siswa_${siswa.nisn}`).showModal()} className="w-6 h-6 flex items-center justify-center  text-white bg-blue-600 hover:bg-blue-800" title="Informasi lebih lanjut">
                                <FontAwesomeIcon icon={faFile} className="w-3 h-3 text-inherit" />
                            </button>
                            <dialog id={`informasi_siswa_${siswa.nisn}`} className="modal">
                            ...
                            <button type="button" onClick={() => deleteSingleSiswa(siswa.nisn)} className="w-6 h-6 flex items-center justify-center  text-white bg-red-600 hover:bg-red-800" title="Hapus data siswa ini?">
