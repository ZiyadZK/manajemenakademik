                ...
                didOpen: async () => {
                    ...
                    if(response.success) {
                        await logRiwayat({
                            aksi: 'Tambah',
                            kategori: 'Data Siswa',
                            keterangan: `Menambah ${dataImport.length} data`,
                            ...
                            if(responseDelete.success) {
                                ...
                                if(response.success) {
                                    await logRiwayat({
                                        aksi: 'Mutasi',
                                        kategori: 'Data Siswa',
                                        keterangan: `Menonaktifkan 1 Data ke Data Mutasi Siswa`,
                                        records: JSON.stringify({...dataSiswa, ...payload})
                                    })
                                    setSelectAll(false)
                                    setSelectedData([])
                                    await getData()
                ...
                if(response.success) {
                    if(kriteria === 'naik') {
                        await logRiwayat({
                            aksi: 'Naikkan Kelas',
                            kategori: 'Data Siswa',
                            keterangan: `Menaikkan Kelas Siswa dari Data Siswa kelas ${kelas} untuk nis ${nis}`
                        })
                    }else{
                        await logRiwayat({
                            aksi: 'Turunkan Kelas',
                            kategori: 'Data Siswa',
                            keterangan: `Menurunkan Kelas Siswa dari Data Siswa kelas ${kelas} untuk nis ${nis}`
                        })
                    }
                    setSelectAll(false)
                    setSelectedData([])
                ...
                if(response.success) {
                    if(type === 'semua kelas') {
                        await logRiwayat({
                            aksi: 'Naikkan Kelas',
                            kategori: 'Data Siswa',
                            keterangan: `Menaikkan Semua Kelas Data Siswa`
                        })
                    }else if(type === 'kecuali') {
                        await logRiwayat({
                            aksi: 'Naikkan Kelas',
                            kategori: 'Data Siswa',
                            keterangan: `Menaikkan Semua Kelas Data Siswa kecuali ${selectedData.length} Siswa`,
                        ...
                        await logRiwayat({
                            aksi: 'Naikkan Kelas',
                            kategori: 'Data Siswa',
                            keterangan: `Menaikkan Kelas hanya untuk ${selectedData.length} Siswa`,
                            records: JSON.stringify({nis: selectedData})
                        })
                    }
                    setSelectAll(false)
                    ...
                    Swal.fire({
                        ...
                        didOpen: async () => {
                            await logRiwayat({
                                aksi: 'Export',
                                kategori: 'Data Siswa',
                                keterangan: `Mengexport ${dataSiswa.length} Data Siswa sebagai EXCEL`,
        ...
        if(exportAll) {
            if(selectedData.length < 1) {
                await logRiwayat({
                    aksi: 'Export',
                    kategori: 'Data Siswa',
                    keterangan: `Mengexport ${data.length} Data Siswa sebagai EXCEL`,
                ...
                await logRiwayat({
                    aksi: 'Export',
                    kategori: 'Data Siswa',
                    keterangan: `Mengexport ${dataImport.length} Data Siswa sebagai Excel`,
